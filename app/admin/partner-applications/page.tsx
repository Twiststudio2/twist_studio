...
import { supabase, type PartnerApplication, type Profile } from '@/lib/supabase';
...
import { Check, X, Ban, Trash2, Eye, Users, Filter } from 'lucide-react';

type ApplicationRow = PartnerApplication & {
...
export default function PartnerApplicationsPage() {
  ...
  const updateLocalStatus = (id: string, status: string) => {
    setApplications((prev) =>
      prev.map((a) => (a.id === id ? { ...a, status: status as PartnerApplication['status'] } : a))
    );
    setSelected((prev) => (prev && prev.id === id ? { ...prev, status: status as PartnerApplication['status'] } : prev));
  };
  ...
  const handleApprove = async (app: ApplicationRow) => {
    ...
    await notifyUser(
      ...
      'Congratulations! Your partner application has been approved. You can now access your partner dashboard.',
      'success',
      '/partner'
    );
  ...
  const handleReject = async (app: ApplicationRow) => {
    ...
    if (error) {
      ...
      await notifyUser(
        app.user_id,
        'Application Update',
        'Your partner application was not approved at this time. You may reapply in the future.',
        'warning'
      );
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Review and manage partner signup applications.</p>
        </div>
      </div>
          ...
          {selected && (
            <div className="space-y-5">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <DetailField label="Occupation" value={selected.occupation} />
                <DetailField label="Region" value={selected.region} />
                <DetailField label="Town / City" value={selected.town_city} />
                <DetailField label="Emergency Contact" value={selected.emergency_contact} />
                <DetailField
                  label="Confirmed Communicator"
                  value={selected.confirmed_communicator ? 'Yes' : 'No'}
                />
              </div>

              <div className="space-y-2">
                <div className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Why Join</div>
              ...
              <div className="space-y-2">
                <div className="text-xs font-medium uppercase tracking-wider text-muted-foreground">How They Can Help</div>
                <div className="rounded-lg border border-border/60 bg-muted/30 p-3 text-sm">
                  {selected.how_help || '—'}
                </div>
              </div>
              ...
              {selected.profile_picture_url && (
                <div className="space-y-2">
                  <div className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Profile Picture</div>
